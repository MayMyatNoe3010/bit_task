...
import '../../features/to_do/data/models/task.dart';

class NotificationServices{
  static final FlutterLocalNotificationsPlugin _localNotiPlugin = FlutterLocalNotificationsPlugin();
  ...
  static Future<void> initializeNotifications() async{
    ...
    await _localNotiPlugin.initialize(initializationSettings,);
       // onDidReceiveNotificationResponse: onDidReceiveNotificationResponse);
  }
  static Future<void> showReminderNoti(Task task) async {
    const androidDetails = AndroidNotificationDetails(
        Constants.task_reminder_channel, 'Task Reminder',
    ...
    tz.TZDateTime tzDateTime = DateTime.parse(task.startTime).getTZTime(task.reminderTime);
    if(task.isRepeated){
      ...
      await _localNotiPlugin.zonedSchedule(
        ...
        tzDateTime,
        platformDetails,
        matchDateTimeComponents: DateTimeComponents.time,  // Repeat every day at the same time
        androidScheduleMode: AndroidScheduleMode.alarmClock,  // Repeat every 24 hours
      );
